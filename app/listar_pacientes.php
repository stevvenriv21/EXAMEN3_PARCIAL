...
<body>
    <h1><i class="bi bi-people-fill"></i> Lista de Usuarios</h1>

    <table id="tabla_usuario" class="table align-middle text-center" style="width:90%; margin:0 auto 40px auto; border-collapse: separate; border-spacing: 0 8px;">
    <thead>
    ...
    <tbody>
        <?php foreach ($pacientes as $user): ?>
            <tr id="fila-<?php echo $user['id']; ?>" style="background: #2a2a2a; transition: all 0.3s; box-shadow: 0 2px 6px #0004; border-radius:8px; margin-bottom:8px;">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('btn-actualizar-usuario').addEventListener('click', () => {
            ...
            fetch('actualizar_paciente.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id, nombre, correo, telefono, fecha_nacimiento })
            })
            .then(res => res.json())
            .then(data => {
                if(data.success) {
